...
class LIF(nn.Module):
    def __init__(self, tau=1.0, n_steps=10, nnode=0, v_threshold=1.0, v_reset=0., alpha=1.0, surrogate='arctan'):
        super().__init__()
        self.v_threshold = v_threshold
        self.v_reset = v_reset
        self.surrogate = SURROGATE.get(surrogate)
        self.register_parameter("tau", nn.Parameter(
            torch.as_tensor(0.0, dtype=torch.float32)))
        self.register_parameter("vth", nn.Parameter(
            torch.as_tensor(0.0, dtype=torch.float32)))
        self.register_buffer("alpha", torch.as_tensor(
            alpha, dtype=torch.float32))
        self.learnable_tau = torch.nn.Parameter(torch.randn(nnode, nnode))
        self.learnable_vth = torch.nn.Parameter(torch.randn(n_steps))
    ...
    def forward(self, dv):
        ...
        v_th = torch.sigmoid(self.learnable_vth[self.timestep])
        tau = torch.sigmoid(self.tau)
        
        self.v = (1 - tau) * self.v + dv * tau
        spike = self.surrogate(self.v, v_th, self.alpha)
        self.v = (1 - spike) * self.v + spike * (tau * self.v + self.v_reset)
